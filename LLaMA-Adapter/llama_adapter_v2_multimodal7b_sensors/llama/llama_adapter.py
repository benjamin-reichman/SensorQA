...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, llama_ckpt_dir, llama_tokenizer,
        ...
        # self.clip_proj1 = nn.Linear(324000, 324000//32)
        # self.clip_proj2 = nn.Linear(324000//32, v_embed_dim)
        self.clip_proj1 = nn.Linear(225, 450)
        self.clip_proj2 = nn.Linear(450, v_embed_dim)
    ...
    def forward_visual(self, sensors, sensors_mask=None):
        # clip_feats = self.clip_encode_image(sensors)
        clip_feats = self.clip_proj_norm(self.clip_proj2(self.clip_proj1(sensors.view(-1, 12*1440,225).float())))

        sensor_query = self.visual_query.weight.unsqueeze(0).repeat(len(sensors), 1, 1)
        sensor_query_mask = torch.ones_like(sensor_query).bool()
        sensor_query = torch.cat([sensor_query, clip_feats], dim=1)

        if sensors_mask is not None:
            if sensors_mask.ndim == 4:
                sensors_mask = sensors_mask.squeeze(0)
            sensors_mask = torch.cat([sensor_query_mask, sensors_mask], dim=1)
            sensors_mask = (sensors_mask.sum(dim=2) != 768).unsqueeze(2).repeat(1, 1, sensors_mask.shape[1])


        for block in self.visual_blocks:
            sensor_query = block(sensor_query, mask=sensors_mask)

        sensor_query = sensor_query[:, :self.query_len, :]
        sensor_query = self.visual_proj(sensor_query)
        sensor_query = self.visual_proj_norm(sensor_query)

        return sensor_query

    def forward(self, tokens, labels, sensors, sensors_mask=None):
        visual_query = self.forward_visual(sensors, sensors_mask)

        _bsz, seqlen = tokens.shape

        h = self.llama.tok_embeddings(tokens.cuda())
        ...
        if labels.sum() == 0:
            ...
            c_loss = self.criterion(output.reshape(-1, self.llama.vocab_size), labels.flatten().cuda())
    ...
    @torch.inference_mode()
    def generate(
        self, sensors, prompts, mask=None,
        max_gen_len: int = 256,
        ...
        assert len(sensors) == len(prompts)

        with torch.cuda.amp.autocast():
            visual_query = self.forward_visual(sensors, mask)
