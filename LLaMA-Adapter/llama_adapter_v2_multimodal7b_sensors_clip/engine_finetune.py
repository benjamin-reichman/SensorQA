...
def train_one_epoch(model: LLaMA_adapter,
    ...
    for data_iter_step, (examples, labels, example_mask, sensors, sensors_mask) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        # we use a per iteration (instead of per epoch) lr scheduler
        if data_iter_step % accum_iter == 0:
            lr_sched.adjust_learning_rate(optimizer, data_iter_step / len(data_loader) + epoch, args)

        sensors = sensors.to(device, non_blocking=True)
        sensors_mask = sensors_mask.to(device, non_blocking=True)
        with torch.cuda.amp.autocast():
            c_loss, m_loss = model(examples, labels, sensors, sensors_mask)
